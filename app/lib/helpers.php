...
// Helpers compartidos por el sitio publico y el admin. Fusiona lo que antes vivia
// duplicado en public/articulos/lib/helpers.php y public/admin/lib/helpers.php —
// esa duplicacion era la causa de que admin/rag/index.php tuviera que redeclarar
// markdown_render()/ip_cliente()/parse_pg_vector() localmente para no chocar.
...
// Clases Tailwind reutilizadas en todas las vistas (público y admin) — un solo
// lugar para ajustar el look & feel (botones, campos, tarjetas) sin tocar cada
// plantilla.
function tw_btn(string $variante = 'primario'): string
{
    return match ($variante) {
        ...
        'outline-danger' => 'inline-flex items-center justify-center gap-2 rounded border border-red-400 px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-500 hover:text-white transition',
        'outline-sm' => 'inline-flex items-center justify-center gap-2 rounded border border-marca-azul px-3 py-1 text-xs font-semibold text-marca-azul hover:bg-marca-azul hover:text-white transition',
        'outline-danger-sm' => 'inline-flex items-center justify-center gap-2 rounded border border-red-400 px-3 py-1 text-xs font-semibold text-red-600 hover:bg-red-500 hover:text-white transition',
        default => '',
    };
}
